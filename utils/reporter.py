...
def build_report_manager(opt):
    if opt.tensorboard:
        ...
        writer = SummaryWriter(opt.tensorboard_log_dir + datetime.now().strftime("/%b-%d_%H-%M-%S"), comment="Unmt")
    else:
        writer = None
...
class Statistics(object):
    ...
    * elapsed time
    ...
    def __init__(self, loss=0, normalization=0, n_correct=0):
        self.loss = loss
    ...
    def accuracy(self):
        """ compute accuracy """
        return 100 * (self.n_correct / self.normalization)

    def cls_loss(self):
        """ compute classification loss """
        return self.loss / self.normalization

    def ppl(self):
        """ compute perplexity """
        return math.exp(min(self.loss / self.normalization, 100))
    ...
    def output(self, step, num_steps, learning_rate, start):
        ...
        """
        t = self.elapsed_time()
        logger.info(f"Step {step:2d}/{num_steps:5d} | ACC: {self.accuracy():.3f}% | CLS_LOSS: {self.cls_loss():.6f} | LR: {learning_rate:.8f}")
        sys.stdout.flush()

    def log_tensorboard(self, prefix, writer, learning_rate, step):
        """ display statistics to tensorboard """
        t = self.elapsed_time()
        writer.add_scalar(prefix + "/cls_loss", self.cls_loss(), step)
